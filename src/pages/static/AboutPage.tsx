...
const AboutPage: FC = () => {
  ...
  <div className="max-w-[1480px] mx-auto px-[clamp(20px,4vw,56px)] pb-[100px]">
    ...
    <div className="py-[72px] pb-14 border-b border-rule mb-16 max-[640px]:pt-12 max-[640px]:pb-10 max-[640px]:mb-10">
      <span className="font-body text-[11px] tracking-[.2em] text-muted uppercase mb-3.5 block">ABOUT LUXERA</span>
    ...
    <div className="grid grid-cols-[1.2fr_1fr] gap-20 items-start mb-[72px] max-[900px]:grid-cols-1 max-[900px]:gap-10">
      <div>
        <h2 className="font-heading font-bold text-[clamp(28px,3vw,44px)] m-0 mb-5 text-ink">چرا لوکسرا؟</h2>
        <p className="text-[15px] leading-[1.9] text-ink-2 m-0 mb-4">بازار جواهرات فانتزی ایران پر از محصولاتی است که سریع رنگ می‌دهند، پوست را حساس می‌کنند، یا با تصویر و توضیحاتشان فرق دارند. لوکسرا با این مشکل شروع شد.</p>
        <p className="text-[15px] leading-[1.9] text-ink-2 m-0 mb-4">هر برندی که در لوکسرا می‌فروشیم پیش از ورود به فهرست، آزمایش می‌شود. نمونه‌ها استفاده می‌شوند، با آب و عرق و عطر تست می‌شوند، و اگر استاندارد ما را نداشتند، نمی‌آیند.</p>
        <p className="text-[15px] leading-[1.9] text-ink-2 m-0 mb-4">نتیجه؟ یک مجموعه‌ی کوچک اما قابل اعتماد از جواهراتی که می‌توانید با خیال راحت بخرید، بپوشید، و هدیه بدهید.</p>
    ...
    <div className="mb-16 pt-14 border-t border-rule">
      <span className="font-body text-[11px] tracking-[.2em] text-muted uppercase mb-9 block">ارزش‌های ما</span>
      {VALUES.map((v) => (
    ...
    <div className="grid grid-cols-4 border border-rule mb-16 max-[900px]:grid-cols-2 max-[640px]:grid-cols-2">
      {STATS.map((s, idx) => (
        <div key={s.label} className={`py-9 px-6 text-center border-r border-rule last:border-r-0 max-[900px]:border-b max-[900px]:border-rule ${idx % 2 === 1 ? 'max-[900px]:border-r-0' : ''}`}>
          <strong className="block font-heading text-[clamp(28px,3vw,40px)] text-ink mb-1.5 not-italic">{s.value}</strong>
    ...
    <div className="text-center bg-plate py-14 px-[clamp(20px,4vw,56px)] -mx-[clamp(20px,4vw,56px)] -mb-[100px] mt-16 max-[640px]:py-11">
      ...
      <p className="text-muted text-sm m-0 mb-7">تیم پشتیبانی لوکسرا روزانه از ساعت ۱۰ تا ۲۲ پاسخگوی شماست.</p>
