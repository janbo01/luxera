...
const PILL_BASE =
  'inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full font-mono text-[10px] tracking-[.1em] uppercase font-medium whitespace-nowrap before:content-[""] before:w-[5px] before:h-[5px] before:rounded-full before:bg-current before:shrink-0'

const IC_MD =
  'w-[38px] h-[38px] rounded-[10px] bg-bg-2 grid place-items-center text-copper shrink-0 [&_svg]:w-[18px] [&_svg]:h-[18px]'
const IC_CIR =
  'w-8 h-8 rounded-full bg-bg-2 grid place-items-center text-copper shrink-0 [&_svg]:w-3.5 [&_svg]:h-3.5'
const IC_SECT =
  'w-8 h-8 rounded-full bg-bg-2 grid place-items-center text-copper shrink-0 [&_svg]:w-4 [&_svg]:h-4'

const AVA_SM =
  'w-[38px] h-[38px] rounded-full bg-gradient-to-br from-plum to-copper grid place-items-center text-white font-heading font-bold text-sm shrink-0 overflow-hidden'
const AVA_MD =
  'w-16 h-16 rounded-full bg-gradient-to-br from-plum to-copper grid place-items-center text-white font-heading font-bold text-2xl shrink-0 shadow-[0_0_0_4px_var(--color-bg),0_0_0_5px_var(--color-rule)] overflow-hidden'
const AVA_LG =
  'w-[72px] h-[72px] rounded-full bg-gradient-to-br from-plum to-copper grid place-items-center text-white font-heading font-bold text-[28px] shrink-0'

const INPUT_CLS =
  'bg-bg border border-rule rounded-[10px] px-4 py-3 text-sm text-ink outline-none w-full transition-[border-color] focus:border-ink placeholder:text-muted placeholder:font-light disabled:opacity-50 disabled:cursor-not-allowed'
...
const BTN_PRIMARY =
  'inline-flex items-center gap-2.5 px-[22px] py-[13px] rounded-full bg-ink text-bg font-medium text-sm border-0 cursor-pointer transition-[transform,background] hover:bg-plum hover:-translate-y-px font-body whitespace-nowrap'
const BTN_MUTE =
  'inline-flex items-center gap-2.5 px-[22px] py-[13px] rounded-full bg-bg-2 text-ink-2 font-medium text-sm border border-rule cursor-pointer transition-all hover:bg-bg hover:text-ink hover:border-ink font-body whitespace-nowrap'
...
function illusSvg(illus: string) {
  ...
  if (illus?.includes('bracelet') || illus?.includes('star'))
    return (
      <>
        <ellipse cx="50" cy="50" rx="30" ry="13" />
        <ellipse cx="50" cy="50" rx="24" ry="9" />
      </>
    )
  if (illus?.includes('ring') || illus?.includes('rose'))
    return (
      <>
        ...
        <path d="M44 41 L50 33 L56 41 Z" />
      </>
    )
  if (illus?.includes('earring') || illus?.includes('diamond'))
    return (
      <>
        <path d="M35 25c0 6 6 10 6 16M65 25c0 6 6 10 6 16" />
...
const OverviewPane: FC<PaneProps> = ({ setTab }) => {
  ...
  const kpis = useMemo(
    () => [
      {
        icon: <IcoBag />,
        value: toFa(orders.length),
        label: 'سفارش‌های کل',
        extra: thisMonthOrders > 0 ? `+${toFa(thisMonthOrders)} این ماه` : null,
      },
      {
        icon: <IcoTruck />,
        value: toFa(inTransit),
        label: 'در حالِ ارسال',
        extra: inTransit > 0 ? 'در راه' : null,
      },
      {
        icon: <IcoHeart />,
        value: toFa(wishCount),
        unit: 'قطعه',
        label: 'علاقه‌مندی‌ها',
        extra: null,
      },
      {
        icon: <IcoStar />,
        value: loyaltyBalance === null ? '…' : toFa(loyaltyBalance),
        unit: loyaltyBalance !== null ? 'امتیاز' : undefined,
        muted: loyaltyBalance === null,
        label: 'امتیازِ شما',
        extra: null,
      },
    ],
    ...
    <div className="flex flex-col gap-[18px]">
      ...
      <div className="grid grid-cols-4 gap-3.5 max-[1100px]:grid-cols-2">
        {kpis.map((k, i) => (
          <div
            key={i}
            className="bg-surface border border-rule rounded-[var(--radius)] px-[22px] py-5 flex flex-col gap-4 relative overflow-hidden after:content-[''] after:absolute after:-left-[10%] after:-bottom-[30%] after:w-[60%] after:h-[60%] after:bg-[radial-gradient(circle,rgba(196,135,58,.06),transparent_60%)] after:pointer-events-none"
          >
            <div className="flex justify-between items-start gap-2">
              <span className={IC_MD}>{k.icon}</span>
              {k.extra && (
                <span className="font-mono text-[10px] text-ok bg-ok/10 px-[7px] py-[3px] rounded-full tracking-[.04em]">
                  {k.extra}
                </span>
              )}
            </div>
            <div className="flex flex-col gap-1">
              <div
                className={`font-heading text-[30px] font-bold leading-none flex items-baseline gap-[5px] ${k.muted ? 'text-muted text-[22px]' : 'text-ink'}`}
              >
                {k.value}
                {k.unit && (
                  <small className="font-body text-[13px] font-normal text-muted mr-0.5">
                    {k.unit}
                  </small>
                )}
              </div>
      ...
      <div className="grid grid-cols-[1.5fr_1fr] gap-[18px] max-[1100px]:grid-cols-1">
        ...
        <div className={CARD}>
          <div className={CARD_H}>
            <h2 className={CARD_H3}>
              <span className={IC_CIR}>
                <IcoClock />
              </span>{' '}
              سفارش‌های اخیر
            </h2>
            <button
              className="text-[12px] text-copper font-body bg-transparent border-0 cursor-pointer hover:underline hover:underline-offset-[3px]"
          ...
          <div className="flex flex-col">
            {orders.slice(0, 3).map((order) => (
              <div
                key={order.id}
                className="grid grid-cols-[auto_1fr_auto_auto] gap-4 items-center px-6 py-3.5 border-b border-rule last:border-b-0"
              >
                <div className="flex">
                  {order.items.slice(0, 2).map((item, i) => (
                    <span
                      key={i}
                      className="w-[42px] h-[42px] rounded-[9px] bg-bg-2 grid place-items-center text-ink-2 border-2 border-surface -mr-3 first:mr-0 [&_svg]:w-[34px] [&_svg]:h-[34px]"
                    >
                      <svg
                        viewBox="0 0 100 100"
                        fill="none"
                ...
                <div className="flex flex-col gap-0.5 min-w-0">
                  ...
                  <span className="font-heading text-sm font-semibold leading-[1.3] whitespace-nowrap overflow-hidden text-ellipsis">
                    {order.items[0].name}
                    {order.items.length > 1 ? ` + ${toFa(order.items.length - 1)} قطعه` : ''}
                  </span>
                </div>
                <span className={`${PILL_BASE} ${PILL_COLORS[STATUS_PILL[order.status]]}`}>
                  {STATUS_FA[order.status]}
                </span>
                <span className="font-heading text-sm font-bold whitespace-nowrap">
                  {formatToman(order.total)}
                  <small className="text-[11px] font-normal text-muted mr-[3px]">ت</small>
                </span>
              </div>
            ))}
            {orders.length === 0 && (
              <div className="py-8 px-6 text-center text-muted text-sm">
        ...
        <div className={CARD}>
          ...
          <div className="p-2.5">
            {[
              ...
              {
                icon: <IcoReorder />,
                title: 'درخواستِ بازگشت',
                desc: '۴ روز فرصت دارید',
                tab: 'orders' as Tab,
              },
              {
                icon: <IcoInfo />,
                title: 'تماس با پشتیبانی',
                desc: 'پاسخگویی روزانه ۹–۲۱',
                tab: null,
              },
              {
                icon: <IcoInvoice />,
                title: 'گارانتی و اصالت',
                desc: 'گواهی‌های دیجیتالِ شما',
                tab: null,
              },
            ].map((item, i) => (
              <button
                key={i}
                className="flex items-center gap-3 px-3.5 py-3 rounded-[10px] transition-colors w-full text-right hover:bg-bg-2"
                onClick={() => item.tab && setTab(item.tab)}
              >
                <span className="w-9 h-9 rounded-lg bg-bg-2 grid place-items-center text-copper shrink-0 [&_svg]:w-4 [&_svg]:h-4">
                  {item.icon}
                </span>
                <div className="flex-1 min-w-0">
                  <div className="font-heading text-[13px] font-semibold leading-[1.2]">
                    {item.title}
                  </div>
                  <div className="text-[11px] text-muted mt-0.5 leading-[1.3]">{item.desc}</div>
                </div>
                <span className="text-muted [&_svg]:w-3.5 [&_svg]:h-3.5">
                  <IcoArrowLeft />
                </span>
              </button>
            ))}
    ...
    <div className="flex flex-col gap-[18px]">
      ...
      <div className={CARD}>
        <div className={SECTION_H}>
          <h2 className="font-heading text-[18px] font-semibold m-0 flex items-center gap-3">
            <span className={IC_SECT}>
              <IcoUser />
            </span>{' '}
            اطلاعاتِ شخصی
          </h2>
          <p className="m-0 mt-1.5 text-[13px] text-muted mr-11">
            این اطلاعات روی فاکتورها و رسیدِ تحویلِ سفارش استفاده می‌شود.
          </p>
        </div>
        <div className="flex items-center gap-[18px] px-7 py-5 bg-surface-2 border-b border-rule">
          ...
          <div className="flex-1">
            ...
            <p className="m-0 mt-0.5 text-[12px] text-muted">
              JPG یا PNG · حداکثر ۲ مگابایت · ۴۰۰×۴۰۰ پیکسل
            </p>
          </div>
          <div className="flex gap-2">
            ...
            {profile?.avatarUrl && (
              <button
                className={`${BTN_MUTE} ${BTN_SM} text-sale`}
                disabled={avatarLoading}
          ...
          <div className="grid grid-cols-2 gap-[18px]">
            <div className={F_FIELD}>
              <label className={F_LABEL}>
                نام <span className="text-sale">*</span>
              </label>
              <input
                className={INPUT_CLS}
                value={form.name}
            ...
            <div className={F_FIELD}>
              <label className={F_LABEL}>
                نامِ خانوادگی <span className="text-sale">*</span>
              </label>
              <input
                className={INPUT_CLS}
                value={form.family}
            ...
            <div className={F_FIELD}>
              <label className={F_LABEL}>
                ایمیل{' '}
                <span className="mr-auto inline-flex items-center gap-1 font-mono text-[10px] text-ok tracking-[.04em] [&_svg]:w-[11px] [&_svg]:h-[11px]">
                  <IcoCheck />
                  تأیید شده
                </span>
              </label>
            ...
            <div className={F_FIELD}>
              <label className={F_LABEL}>
                تاریخِ تولد{' '}
                <span className="mr-auto font-mono text-[10px] text-muted font-normal tracking-[.04em]">
                  (اختیاری)
                </span>
              </label>
              <DatePicker
                ...
                onChange={(date: DateObject | null) =>
                  setForm((f) => ({
                    ...f,
                    birthdate: date ? date.toDate().toISOString().split('T')[0] : '',
                  }))
                }
            ...
            <div className={`${F_FIELD} col-span-2`}>
              <label className={F_LABEL}>
                کدِ ملی{' '}
                <span className="mr-auto font-mono text-[10px] text-muted font-normal tracking-[.04em]">
                  (برای صدورِ فاکتورِ رسمی)
                </span>
              </label>
              <input
                className={INPUT_CLS}
                placeholder="۰۰۸۷۶۵۴۳۲۱"
        ...
        <div className="px-7 py-[18px] bg-surface-2 border-t border-rule flex justify-between items-center gap-3">
          <span className="text-[11px] text-muted">
            آخرین تغییر:{' '}
            {profile?.updatedAt ? new Date(profile.updatedAt).toLocaleDateString('fa-IR') : '—'}
          </span>
          <div className="flex gap-2">
      ...
      <div className={CARD}>
        <div className={SECTION_H}>
          ...
          <p className="m-0 mt-1.5 text-[13px] text-muted mr-11">
            کنترل کنید چه پیام‌هایی از ما دریافت کنید.
          </p>
        </div>
        <div className={SECTION_B}>
          {(
            [
              {
                ...
                desc: 'هر دو هفته یک ایمیل با جدیدترین قطعات و پیشنهادها',
              },
              {
                key: 'sms',
                icon: <IcoPhone />,
                title: 'پیامکِ سفارش',
                desc: 'وضعیتِ ارسال و تحویل از طریقِ SMS',
              },
              {
                key: 'offers',
                icon: <IcoStar />,
                title: 'پیشنهادهای ویژه',
                desc: 'دسترسیِ زودهنگام و تخفیف‌های اعضای طلایی',
              },
              {
                key: 'whatsapp',
                icon: <IcoWhatsApp />,
                title: 'پیام‌رسانِ واتس‌اپ',
                desc: 'پشتیبانی و اطلاع‌رسانی از طریقِ واتس‌اپ',
              },
            ] as const
          ).map(({ key, icon, title, desc }) => (
            <div
              key={key}
              className="flex items-start gap-3.5 py-3.5 border-t border-rule first:border-t-0 first:pt-0"
            >
              <span className="w-[34px] h-[34px] rounded-full bg-bg-2 grid place-items-center text-copper shrink-0 [&_svg]:w-[15px] [&_svg]:h-[15px]">
                {icon}
              </span>
              <div className="flex-1">
...
const OrdersPane: FC = () => {
  ...
  const filtered = useMemo(() => {
    let result = orders
    if (filter === 'transit')
      result = result.filter((o) => o.status === 'shipped' || o.status === 'processing')
  ...
  const chips = useMemo(
    () => [
      { key: 'all', label: 'همه', count: orders.length },
      {
        key: 'transit',
        label: 'در راه',
        count: orders.filter((o) => o.status === 'shipped' || o.status === 'processing').length,
      },
      {
        key: 'delivered',
        label: 'تحویل شده',
            ...
            <button
              ...
              className={`px-[13px] py-[7px] rounded-full text-[12px] border transition-all cursor-pointer inline-flex items-center gap-1.5 font-body ${
                filter === c.key
                  ...
                  : 'bg-bg text-ink-2 border-rule hover:border-ink'
              }`}
      ...
      <div className="flex flex-col gap-3">
        ...
        {filtered.map((order) => {
          ...
          return (
            <article
              key={order.id}
              className="bg-surface border border-rule rounded-[var(--radius)] overflow-hidden"
            >
              {/* Head */}
              <div
              ...
              >
                <div className="flex max-[1100px]:hidden">
                  {order.items.slice(0, 2).map((item, i) => (
                    <span
                      key={i}
                      className="w-[42px] h-[42px] rounded-[9px] bg-bg-2 grid place-items-center text-ink-2 border-2 border-surface -mr-3 first:mr-0"
                    >
                      <svg
                        viewBox="0 0 100 100"
                        fill="none"
                ...
                <div className="flex flex-col gap-0.5 min-w-0">
                  <span
                    className="font-mono text-[13px] font-semibold tracking-[.04em]"
                    dir="ltr"
                  ...
                  <span className="text-[11px] text-muted font-mono tracking-[.04em]">
                    {order.date} · {toFa(order.items.length)} قلم
                  </span>
                </div>
                <span className={`${PILL_BASE} ${PILL_COLORS[STATUS_PILL[order.status]]}`}>
                  {STATUS_FA[order.status]}
                ...
                <span
                  className={`w-9 h-9 rounded-full grid place-items-center transition-[transform,background,color] shrink-0 [&_svg]:w-3.5 [&_svg]:h-3.5 ${
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-2.5">
                        {order.items.map((item, i) => (
                          <div
                            key={i}
                            className="grid grid-cols-[54px_1fr_auto] gap-3.5 items-center p-3 px-3.5 bg-surface border border-rule rounded-[10px]"
                          >
                            ...
                            <div className="flex flex-col gap-0.5 min-w-0">
                              <span className="font-heading text-[13px] font-semibold leading-[1.3]">
                                {item.name}
                              </span>
                              <span className="text-[11px] text-muted font-mono tracking-[.04em]">
                                قطعه‌ی جواهری
                      ...
                      <div className="flex gap-2 mt-3.5 flex-wrap">
                        <button
                          className={`${BTN_PRIMARY} ${BTN_SM} [&_svg]:w-[13px] [&_svg]:h-[13px]`}
                          onClick={() => {
                            reorderItems(order, addItem)
                            openCart()
                          }}
                        >
                          <IcoReorder /> سفارشِ مجدد
                        </button>
                        <button
                          className={`${BTN_MUTE} ${BTN_SM} [&_svg]:w-[13px] [&_svg]:h-[13px]`}
                          disabled
                          title="API موجود نیست"
                        >
                          <IcoInvoice /> دانلود فاکتور
                        </button>
                        <button
                          className={`${BTN_MUTE} ${BTN_SM} [&_svg]:w-[13px] [&_svg]:h-[13px]`}
                          disabled
                    ...
                    <div>
                      <div className="bg-surface border border-rule rounded-[10px] p-[18px_20px]">
                        ...
                        {steps.map((step, i) => {
                          ...
                          return (
                            <div
                              key={i}
                              className="grid grid-cols-[auto_1fr] gap-3.5 pb-3.5 relative last:pb-0"
                            >
                              <span
                                className={`w-[22px] h-[22px] rounded-full border-[1.5px] grid place-items-center shrink-0 z-[1] [&_svg]:w-[11px] [&_svg]:h-[11px] ${
                                  isDone
                              ...
                              {i < steps.length - 1 && (
                                <span
                                  className={`absolute right-[11px] top-6 bottom-0 w-[1.5px] ${isDone ? 'bg-ok' : 'bg-rule'}`}
                                />
                              )}
                              <div>
                                <div
                                  className={`font-heading text-[13px] font-semibold leading-[1.2] ${isDone ? '' : 'text-muted'}`}
                                >
                                  {step}
                                </div>
                                <div
                                  className="font-mono text-[10px] text-muted tracking-[.04em] mt-0.5"
                                  dir="ltr"
                                  style={{ textAlign: 'right' }}
                                >
                                  {order.date}
                      ...
                      {order.trackingCode && (
                        <div className="mt-2.5 px-3.5 py-[11px] bg-surface border border-rule rounded-[10px] flex items-center gap-2.5">
                          <span className="text-[10px] text-muted font-mono tracking-[.08em] uppercase">
                            رهگیری
                          </span>
                          <span
                            className="font-mono text-[13px] font-semibold flex-1 tracking-[.04em]"
                            dir="ltr"
                            style={{ textAlign: 'right' }}
                          >
                            {order.trackingCode}
...
const AddressModal: FC<AddressModalProps> = ({ initial, onClose }) => {
  ...
  const handleSubmit = async () => {
    if (!form.fullName.trim()) {
      setError('نام گیرنده الزامی است')
      return
    }
    if (!form.phone.trim()) {
      ...
      <div className="flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-5 pb-[18px] border-b border-rule">
          <div className="flex items-center gap-2.5">
            <span className="w-8 h-8 rounded-[10px] bg-copper/[.12] grid place-items-center text-copper shrink-0 [&_svg]:w-[15px] [&_svg]:h-[15px]">
              <IcoPin />
            </span>
            <h2 className="m-0 text-[15px] font-heading font-bold text-ink">
              {isEdit ? 'ویرایشِ آدرس' : 'آدرسِ جدید'}
            </h2>
          </div>
          <button
            className="w-[30px] h-[30px] rounded-lg bg-transparent cursor-pointer text-muted text-[13px] grid place-items-center transition-[background,color] hover:bg-bg-2 hover:text-ink"
            onClick={onClose}
            aria-label="بستن"
          >
            ✕
        ...
        <div className="px-6 py-5 overflow-y-auto max-h-[calc(90vh-148px)] flex flex-col gap-[22px]">
          ...
          <div className="flex flex-col gap-2.5">
            <p className="m-0 text-[11px] font-mono tracking-[.1em] uppercase text-muted font-medium">
              اطلاعاتِ گیرنده
            </p>
            <div className="grid grid-cols-2 gap-[18px]">
              <div className={F_FIELD}>
                <label className={F_LABEL}>
                  نام گیرنده <span className="text-sale">*</span>
                </label>
                <input
                  className={INPUT_CLS}
                  value={form.fullName}
              ...
              <div className={F_FIELD}>
                <label className={F_LABEL}>
                  شماره تماس <span className="text-sale">*</span>
                </label>
                <input
                  className={INPUT_CLS}
                  dir="ltr"
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <div className="grid grid-cols-2 gap-[18px]">
              <div className={F_FIELD}>
                <label className={F_LABEL}>
                  استان <span className="text-sale">*</span>
                </label>
                <select
                  className={SELECT_CLS}
                  style={{ backgroundImage: SELECT_BG }}
              ...
              <div className={F_FIELD}>
                <label className={F_LABEL}>
                  شهر <span className="text-sale">*</span>
                </label>
                <select
                  className={SELECT_CLS}
                  style={{ backgroundImage: SELECT_BG }}
              ...
              <div className={`${F_FIELD} col-span-2`}>
                <label className={F_LABEL}>
                  آدرس کامل <span className="text-sale">*</span>
                </label>
                <input
                  className={INPUT_CLS}
                  value={form.street}
              ...
              <div className={F_FIELD}>
                <label className={F_LABEL}>
                  کد پستی <span className="text-sale">*</span>
                </label>
                <input
                  className={INPUT_CLS}
                  dir="ltr"
          ...
          {/* Default toggle */}
          <label
            className={`flex items-center gap-2.5 cursor-pointer py-3 px-3.5 rounded-[10px] border-[1.5px] transition-[border-color,background] select-none ${form.isDefault ? 'border-ink bg-surface' : 'border-rule bg-bg'}`}
          >
            <input
              type="checkbox"
              className="sr-only"
            ...
            />
            <span
              className={`w-[18px] h-[18px] rounded-[5px] border-[1.5px] shrink-0 grid place-items-center transition-all ${form.isDefault ? 'border-ink bg-ink' : 'border-rule bg-bg'}`}
            >
              <span
                className={`w-[9px] h-[9px] rounded-[3px] transition-colors ${form.isDefault ? 'bg-bg' : 'bg-transparent'}`}
              />
            </span>
        ...
        <div className="flex gap-2 flex-row-reverse justify-start px-6 py-4 border-t border-rule">
          <button className={`${BTN_MUTE} ${BTN_SM}`} onClick={onClose} disabled={loading}>
            لغو
          </button>
...
const AddressesPane: FC = () => {
  ...
  return (
    <div className="flex flex-col gap-[18px]">
      {modal.open && (
        <AddressModal initial={modal.address} onClose={() => setModal({ open: false })} />
      )}
      <div className={CARD}>
        <div className={`${SECTION_H} flex justify-between items-end border-b-0`}>
          <div>
            ...
            <p className="m-0 mt-1.5 text-[13px] text-muted mr-11">
              برای ارسالِ سریع‌تر، می‌توانید چند آدرس ذخیره کنید.
            </p>
          </div>
          <button
            className={`${BTN_PRIMARY} ${BTN_SM} [&_svg]:w-[14px] [&_svg]:h-[14px]`}
        ...
        <div className="px-7 py-2 pb-7 max-[640px]:p-4">
          <div className="grid grid-cols-2 gap-3.5 max-[640px]:grid-cols-1">
            {addresses.map((addr) => (
              <div
                key={addr.id}
                className={`relative p-[22px] bg-surface border rounded-[var(--radius)] flex flex-col gap-3.5 transition-[border-color,box-shadow] ${
                  addr.isDefault
              ...
              >
                <div className="flex items-center justify-between gap-2.5">
                  <span
                    className={`inline-flex items-center gap-1.5 px-2.5 py-[5px] rounded-full font-mono text-[10px] tracking-[.12em] uppercase font-medium [&_svg]:w-[11px] [&_svg]:h-[11px] ${
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="font-heading text-[15px] font-semibold flex items-center gap-2">
                    <span className="text-copper inline-grid place-items-center [&_svg]:w-3.5 [&_svg]:h-3.5">
                      <IcoPin />
                    </span>
                    {addr.fullName}
                  </div>
                  <div className="text-[13px] text-ink-2 leading-[1.7]">
                  ...
                  <div className="flex gap-2.5 flex-wrap text-[12px] text-muted pt-1.5 border-t border-dashed border-rule">
                    <span
                      className="inline-flex items-center gap-1.5 font-mono tracking-[.04em] [&_svg]:w-[11px] [&_svg]:h-[11px]"
                      dir="ltr"
                    >
                      <IcoPhone />
                      {toFa(addr.phone)}
                    ...
                    >
                      <IcoCard />
                      {toFa(addr.postalCode)}
                    </span>
                  </div>
                </div>
                <div className="flex justify-between items-center gap-2.5 pt-3 border-t border-rule">
                  {addr.isDefault ? (
                    <button className="text-[12px] text-copper bg-transparent border-0 p-[7px_0] cursor-default opacity-45 font-body">
                      آدرسِ پیش‌فرض
                    </button>
                  ) : (
                    <button
                      className="text-[12px] text-copper bg-transparent border-0 p-[7px_0] cursor-pointer font-body hover:underline hover:underline-offset-[3px]"
                      onClick={() => setDefaultAddress(addr.id)}
                    >
                      تنظیم به‌عنوانِ پیش‌فرض
                    </button>
                  )}
                  <div className="flex gap-1.5">
                    <button
                      className="inline-flex items-center gap-1.5 px-3 py-[7px] rounded-lg text-[12px] bg-bg text-ink-2 border border-rule transition-all cursor-pointer font-body hover:bg-ink hover:text-bg hover:border-ink [&_svg]:w-3 [&_svg]:h-3"
                    ...
                    <button
                      className="inline-flex items-center gap-1.5 px-3 py-[7px] rounded-lg text-[12px] bg-bg text-ink-2 border border-rule transition-all cursor-pointer font-body hover:bg-sale hover:text-white hover:border-sale [&_svg]:w-3 [&_svg]:h-3"
            ...
            <button
              ...
              onClick={() => setModal({ open: true })}
            >
              <span className="w-12 h-12 rounded-full bg-bg-2 grid place-items-center text-copper [&_svg]:w-5 [&_svg]:h-5">
                <IcoPlus />
              </span>
      ...
      <div className={CARD}>
        <div className={`${SECTION_H} flex justify-between items-end`}>
          <div>
            ...
            <p className="m-0 mt-1.5 text-[13px] text-muted mr-11">
              قطعاتی که برای بعد نگه داشته‌اید — {toFa(items.length)} مورد
            </p>
          </div>
          {items.length > 0 && (
            <button
          ...
          {items.length === 0 ? (
            ...
            <div className="grid grid-cols-4 gap-3.5 max-[1100px]:grid-cols-3 max-[640px]:grid-cols-2">
              {items.map((product) => (
                <article
                  key={product.id}
                  className="bg-surface border border-rule rounded-[var(--radius)] overflow-hidden flex flex-col relative transition-[transform,box-shadow] duration-[250ms] group hover:-translate-y-[3px] hover:shadow-[0_16px_40px_-28px_rgba(27,15,29,.25)]"
                >
                  <div className="aspect-square bg-gradient-to-bl from-[#EFE5D5] to-[#E6D8C2] grid place-items-center text-ink-2 relative overflow-hidden">
                    <svg
                      ...
                      className="w-[55%] h-[55%]"
                    >
                      {illusSvg(product.illus)}
                    </svg>
                    <button
                      className="absolute top-2.5 left-2.5 w-8 h-8 rounded-full bg-surface/[.92] grid place-items-center text-sale backdrop-blur-[8px] cursor-pointer [&_svg]:w-3.5 [&_svg]:h-3.5"
                  ...
                  <div className="p-3.5 px-4">
                    ...
                    <h3 className="font-heading text-sm font-semibold leading-[1.3] mt-1">
                      {product.fa}
                    </h3>
                    <div className="flex justify-between items-center mt-2">
                      <span className="font-heading text-sm font-bold">
                        {formatToman(product.price)}
                        <small className="text-[10px] text-muted font-normal mr-[3px]">ت</small>
                      </span>
                      <span
                        className={`font-mono text-[10px] inline-flex items-center gap-1 ${product.oldPrice ? 'text-[#B98223]' : 'text-ok'} before:content-[''] before:w-[5px] before:h-[5px] before:rounded-full before:bg-current`}
                      >
                        موجود
                      </span>
                    </div>
    ...
    <div className="flex flex-col gap-[18px]">
      <div className={CARD}>
        <div className={SECTION_H}>
          ...
          <p className="m-0 mt-1.5 text-[13px] text-muted mr-11">
            تنظیمات امنیتی و رمزِ ورودِ حساب خود را مدیریت کنید.
          </p>
        </div>
        <div className={SECTION_B}>
          {(
            [
              {
                key: 'twofa',
                title: 'تأییدِ دو مرحله‌ای',
                desc: 'ارسالِ کدِ تأیید به موبایل هنگامِ ورود',
              },
              {
                key: 'biometric',
                title: 'ورود بیومتریک',
                desc: 'استفاده از اثرِ انگشت یا تشخیصِ چهره',
              },
              {
                key: 'alerts',
                title: 'اعلانِ ورود جدید',
                desc: 'اطلاع‌رسانی از هر ورودِ جدید به حساب',
              },
            ] as const
          ).map(({ key, title, desc }) => (
            <div
            ...
            >
              ...
              <label className="relative w-9 h-5 shrink-0 cursor-pointer mt-0.5">
                <input
                  ...
                  className="sr-only peer"
                  checked={sec[key]}
                  onChange={(e) => setSec((s) => ({ ...s, [key]: e.target.checked }))}
                />
                <span className="absolute inset-0 bg-bg-2 rounded-full transition-colors peer-checked:bg-ink after:content-[''] after:absolute after:top-0.5 after:right-0.5 after:w-4 after:h-4 after:rounded-full after:bg-white after:transition-transform peer-checked:after:-translate-x-4" />
              </label>
            </div>
      ...
      <div className={CARD}>
        <div className={SECTION_H}>
          <h2 className="font-heading text-[18px] font-semibold m-0 flex items-center gap-3">
            <span className={`${IC_SECT} text-sale`}>
              <IcoLogout />
            </span>{' '}
            خروج از حساب
          </h2>
        ...
        <div className={SECTION_B}>
          <button
            className={`${BTN_MUTE} ${BTN_SM} text-sale [&_svg]:w-[14px] [&_svg]:h-[14px]`}
            onClick={() => logout()}
          >
        ...
        <div className="max-w-[1480px] mx-auto px-[clamp(20px,4vw,56px)]">
          <div className="flex justify-between items-end gap-8 flex-wrap max-[640px]:flex-col max-[640px]:items-start">
            <div className="flex items-center gap-[18px]">
              ...
              <div>
                ...
                <h1 className="mt-1 font-heading text-[clamp(24px,3vw,36px)] font-bold m-0 leading-[1.1] tracking-[-0.01em]">
                  خوش آمدید،{' '}
                  <em className="font-display italic font-normal text-copper">
            ...
            <div className="flex gap-2 flex-wrap items-center">
              <button
                className={`${BTN_MUTE} ${BTN_SM} [&_svg]:w-3.5 [&_svg]:h-3.5`}
                onClick={() => setTab('orders')}
              >
                <IcoClock /> تاریخچه‌ی سفارش‌ها
              </button>
              <button
                className={`${BTN_PRIMARY} ${BTN_SM} [&_svg]:w-3.5 [&_svg]:h-3.5`}
            ...
            <aside className="sticky top-24 bg-surface rounded-[var(--radius)] border border-rule overflow-hidden max-[1100px]:static">
              <div className="py-[18px] px-5 pb-4 border-b border-rule flex items-center gap-3">
                ...
                <div className="flex flex-col min-w-0">
                  <span className="font-heading text-sm font-semibold leading-[1.2] whitespace-nowrap overflow-hidden text-ellipsis">
                    {profile?.name}
                  </span>
                  <span
                    className="font-mono text-[11px] text-muted tracking-[.04em]"
                    dir="ltr"
                    style={{ textAlign: 'right' }}
                  >
                    {toFa(profile?.phone ?? '')}
              ...
              <nav className="p-2 max-[1100px]:flex max-[1100px]:flex-wrap">
                {NAV_ITEMS.map(({ id, icon, label, countKey }) => (
                  ...
                  >
                    <span
                      className={`w-8 h-8 rounded-lg grid place-items-center shrink-0 transition-[color,background] [&_svg]:w-[15px] [&_svg]:h-[15px] ${
                    ...
                    {countKey && counts[countKey] > 0 && (
                      <span
                        className={`font-mono text-[10px] tracking-[.04em] px-[7px] py-[2px] rounded-full ${
                ...
                <button
                  ...
                  onClick={() => logout()}
                >
                  <span className="w-8 h-8 rounded-lg bg-sale/[.08] grid place-items-center text-sale shrink-0 [&_svg]:w-[15px] [&_svg]:h-[15px]">
                    <IcoLogout />
                  </span>
