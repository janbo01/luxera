...
const CheckoutPage: FC = () => {
  ...
  useEffect(() => {
    ...
    getLoyaltyBalance()
      ...
      .catch(() => {})
    getStoreSettings()
      .then((s) => setLoyaltyPointValue(s.loyalty_point_value ?? 0))
      .catch(() => {})
  }, [isLoggedIn])
  ...
  const total =
    subtotal + shippingCost + (giftWrap ? GIFT_WRAP_PRICE : 0) - couponDiscount - loyaltyDiscount
    ...
    <div className="pb-20">
      <div className="max-w-[1480px] mx-auto px-[clamp(20px,4vw,56px)]">
        <div className="flex justify-between items-end gap-8 flex-wrap pt-9 pb-2">
          <div>
            <span className="font-display italic text-[13px] text-copper tracking-[.04em]">
              Checkout
            </span>
            ...
            {STEP_LABELS.map(({ lbl, name }, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    if (isDone) goStep(i as CheckoutStep)
                  }}
                  style={{ cursor: isDone ? 'pointer' : isActive ? 'default' : 'not-allowed' }}
                >
                  <span
                    className={`w-9 h-9 rounded-full border-[1.5px] grid place-items-center font-mono text-sm font-semibold shrink-0 transition-all ${
                      isActive
                  ...
                  >
                    {isDone ? <Icon name="check" size={16} strokeWidth={2.5} /> : toFa(i + 1)}
                  </span>
                  <span className="flex flex-col gap-0.5 min-w-0">
                    <span
                      className={`font-mono text-[10px] tracking-[.16em] uppercase transition-colors ${
                    ...
                    <span
                      className={`font-heading text-[15px] font-semibold leading-[1.2] transition-colors ${
