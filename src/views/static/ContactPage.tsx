...
const INPUT =
  'font-body text-sm text-ink bg-surface border border-rule px-3.5 py-[11px] outline-none transition-[border-color] focus:border-ink resize-y appearance-none'
...
const ContactPage: FC = () => {
  usePageMeta({
    title: 'تماس با لوکسرا و پشتیبانی فروشگاه',
      ...
      <div className="grid grid-cols-[1fr_1.4fr] gap-16 items-start mb-20 max-[900px]:grid-cols-1 max-[900px]:gap-12">
        ...
        <aside className="flex flex-col">
          <div className="py-6 border-b border-rule first:pt-0">
            ...
            <p
              dir="ltr"
              className="text-right font-mono text-[13px] tracking-[.06em] text-ink-2 m-0"
            >
              +98 912 849 4308
            </p>
            <p className="text-sm leading-[1.85] text-ink-2 m-0 mt-2">
              پشتیبانی واتس‌اپ: ۱۰ تا ۲۲ روزانه
            </p>
          </div>
        ...
        <div className="sticky top-[120px] max-[900px]:static">
          {submitted ? (
            <div className="flex flex-col items-center text-center py-14 px-6 gap-4 border border-rule animate-rise">
              <div
                className="w-14 h-14 bg-plum text-bg text-[22px] flex items-center justify-center rounded-full"
                aria-hidden="true"
              >
                ✓
              </div>
