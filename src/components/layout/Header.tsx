        ...
        <div className="flex items-center gap-[18px] flex-wrap">
          <span className="inline-flex items-center gap-2">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" width="13" height="13">
              <path d="M3 7h13l5 5-5 5H3z" strokeWidth="1.6" />
            </svg>
...
function NavLinkItem({
  ...
  onClick,
  mobile,
}: NavLink & { onClick?: () => void; mobile?: boolean }) {
  const cls = mobile
    ? accent
  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-50 bg-bg/86 backdrop-saturate-[160%] backdrop-blur-[14px] border-b border-rule overflow-x-hidden">
        ...
        <div
          className="relative flex items-center justify-between px-[var(--pad)] h-[78px] gap-3"
          dir="ltr"
        >
          {/* Logo — left */}
          <a
          ...
          >
            ...
            <div className="flex flex-col leading-none gap-[3px]">
              <span className="font-display italic font-medium text-[27px] tracking-[0.02em] leading-none text-ink">
                Luxera
              </span>
              <span className="font-mono text-[7.5px] tracking-[0.3em] text-muted uppercase">
                Fine Jewelry
              </span>
            </div>
          </a>
          ...
          <nav
            className="flex-1 flex justify-center max-[1100px]:hidden gap-[24px] items-center text-sm font-normal min-w-0"
            dir="rtl"
          >
            {NAV_LINKS.map((link) => (
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            >
              ...
              <kbd className="ms-auto font-mono text-[10px] bg-bg px-1.5 py-0.5 rounded border border-rule text-ink-2">
                ⌘K
              </kbd>
            </button>
      ...
      >
        ...
        <nav className="flex-1 flex flex-col overflow-y-auto">
          {NAV_LINKS.map((link) => (
            <NavLinkItem key={link.to} {...link} onClick={closeMenu} mobile />
          ))}
        </nav>

        <div className="flex gap-5 items-center px-6 py-[18px] border-t border-rule text-xs text-ink-2">
          <button className="flex items-center gap-1.5" aria-label="تغییر زبان">
            <Icon name="globe" size={14} />
            <span>FA</span>
          </button>
          <button aria-label="پشتیبانی">
            <span>پشتیبانی</span>
          </button>
        </div>
      </div>
