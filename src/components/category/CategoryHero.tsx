...
const CAT_DESC: Record<string, string> = {
  necklaces:
    'از زنجیرهای ظریف روزمره تا گردنبندهای ظریف و شیک — با روکش‌های باکیفیت و طرح‌های روز برای هر سبک.',
...
const CAT_GLYPHS: Record<string, React.ReactNode> = {
  necklaces: (
    <svg
      className="w-[78%] h-[78%] text-[rgba(245,237,224,.45)]"
      viewBox="0 0 200 200"
      fill="none"
      stroke="currentColor"
      strokeWidth=".8"
  ...
  return (
    <>
        ...
        <div className="relative z-[2] px-14 py-12 flex flex-col justify-between gap-8 max-md:px-7 max-md:py-8 max-sm:px-5 max-sm:py-6 max-sm:gap-5">
          ...
          <div>
            <h1 className="font-heading font-bold text-[clamp(36px,8vw,116px)] leading-[.95] tracking-[-0.02em] m-0 text-bg">
              {category.fa}
              <span className="block font-display italic font-normal text-[clamp(13px,1.6vw,22px)] tracking-[0.04em] text-[#E8C9B6] mt-3 leading-none">
                {category.en}
              </span>
            </h1>
            <p className="max-w-[48ch] text-[15px] leading-[1.85] text-[rgba(245,237,224,.75)] mt-6 m-0 font-body max-sm:text-[13.5px] max-sm:leading-[1.75] max-sm:mt-4">
              {CAT_DESC[catId] ?? ''}
            </p>
          </div>
          ...
          <div className="flex items-center gap-[18px] flex-wrap pt-6 border-t border-[rgba(245,237,224,.14)] max-sm:flex-nowrap max-sm:overflow-x-auto max-sm:pt-4 max-sm:[scrollbar-width:none] max-sm:[&::-webkit-scrollbar]:hidden">
            {[
              ...
              <Fragment key={i}>
                {i > 0 && (
                  <div
                    className="w-px h-7 bg-[rgba(245,237,224,.18)] flex-shrink-0 max-sm:hidden"
                    aria-hidden="true"
                  />
                )}
                <div className="flex flex-col gap-0.5 shrink-0">
                  <div className="font-heading text-[22px] font-bold text-bg leading-none">{v}</div>
                  <div className="text-[11px] text-[rgba(245,237,224,.55)] font-mono tracking-[0.08em] uppercase">
                    {l}
                  </div>
                </div>
              </Fragment>
      ...
      <div className="mt-[18px] flex gap-2 items-center pb-2 max-md:pb-4 max-md:border-b max-md:border-rule max-md:mb-2 max-sm:overflow-x-auto max-sm:flex-nowrap max-sm:[margin-inline:calc(-1*var(--pad))] max-sm:[padding-inline:var(--pad)] max-sm:[scrollbar-width:none] max-sm:[&::-webkit-scrollbar]:hidden">
        <span className="font-mono text-[11px] text-muted tracking-[0.14em] uppercase me-1.5 shrink-0">
          سایر دسته‌بندی‌ها
        </span>
        {MAIN_CATS.map((cat) => (
          <Link
