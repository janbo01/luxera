...
const Gallery: FC<GalleryProps> = ({ images, productName = 'محصول' }) => {
  ...
  return (
    <div className="sticky top-[96px] rounded-[var(--radius)] overflow-hidden bg-[linear-gradient(145deg,var(--color-plum)_0%,var(--color-plum-2)_100%)] text-bg isolate grid [grid-template-columns:112px_1fr] h-[calc(100vh-128px)] before:absolute before:inset-x-[-20%] before:bottom-[-40%] before:h-[60%] before:bg-[radial-gradient(50%_60%_at_50%_50%,color-mix(in_srgb,var(--color-plum)_18%,transparent),transparent_70%)] before:blur-[20px] before:-z-[1] before:pointer-events-none max-md:grid-cols-1 max-md:[grid-template-rows:auto_1fr] max-lg:static max-lg:h-auto max-lg:flex max-lg:flex-col">
      {/* pthumbs first so RTL auto-placement puts them in column 1 (RIGHT) */}
      <div className="flex flex-col gap-1.5 overflow-y-auto p-2.5 scrollbar-none max-md:flex-row max-md:overflow-x-auto max-md:px-[22px] max-md:pb-[22px] max-md:py-0 max-lg:order-last max-lg:flex-row max-lg:overflow-x-auto max-lg:overflow-y-hidden max-lg:py-2 max-lg:px-3 max-lg:border-t max-lg:border-bg/[12%]">
        {hasApiImages
          ? galleryImages.map((img, i) => (
              <button
                key={img.id}
                className={`w-full aspect-[1/1.1] flex-shrink-0 rounded-[8px] overflow-hidden border-2 transition-all duration-200 max-md:w-[60px] max-md:h-[74px] max-lg:w-[60px] max-lg:h-[74px] ${i === activeIdx ? 'border-[var(--color-dust-rose)] bg-dust-rose/[12%]' : 'border-transparent hover:border-bg/30'}`}
