...
const PaymentStep: FC<PaymentStepProps> = ({
  ...
  <section className="bg-surface rounded-[var(--radius)] border border-rule px-9 pt-9 pb-8 max-[640px]:px-5 max-[640px]:py-6">
      ...
      {availableOptions.map((p) => {
        ...
        return (
          ...
          >
            {/* Radio */}
            <span
              className={`w-[20px] h-[20px] rounded-full border-[1.5px] bg-white grid place-items-center shrink-0 transition-colors ${isOn ? 'border-ink' : 'border-rule'}`}
            >
              <span
                className={`w-[9px] h-[9px] rounded-full bg-ink transition-transform duration-150 ${isOn ? 'scale-100' : 'scale-0'}`}
              />
            </span>
            {/* Logo */}
            <span
              className={`w-[42px] h-[42px] rounded-[9px] grid place-items-center font-display italic text-[18px] font-medium shrink-0 ${PAY_LOGO_STYLE[p.logoClass] ?? 'bg-bg-2 text-ink-2'}`}
            >
              {p.logoChar}
            </span>
            {/* Info */}
    ...
    <div className="grid grid-cols-4 gap-3 mt-[22px] pt-[22px] border-t border-rule max-[1100px]:grid-cols-2">
      {[
        {
          icon: <Icon name="shield" size={18} strokeWidth={1.8} />,
          title: 'پرداختِ امن',
          sub: 'رمزنگاری ۲۵۶ بیتی SSL',
        },
        {
          icon: <Icon name="truck" size={18} strokeWidth={1.8} />,
          title: 'ارسالِ تضمینی',
          sub: 'بیمه‌ی کاملِ مرسوله',
        },
        {
          icon: <CircleCheck size={18} strokeWidth={1.7} />,
          title: 'اصالتِ کالا',
          sub: 'گارانتی کیفیت همراهِ سفارش',
        },
        {
          icon: <Icon name="refresh" size={18} strokeWidth={1.8} />,
          title: '۴ روز بازگشت',
          sub: 'بدونِ پرسش، بدونِ هزینه',
        },
      ].map(({ icon, title, sub }) => (
        <div key={title} className="flex items-start gap-2.5 text-[12px] text-ink-2 leading-[1.5]">
          ...
          <div>
            <strong className="font-heading text-[13px] font-semibold block text-ink">
              {title}
            </strong>
            {sub}
          </div>
    ...
    {payError && <p className="mt-4 text-[13px] text-sale font-mono">{payError}</p>}
    ...
    <div className="flex justify-between items-center gap-3.5 mt-7 pt-6 border-t border-rule max-[640px]:flex-col-reverse max-[640px]:items-stretch">
      ...
      <button
        className="inline-flex items-center gap-2.5 px-7 py-4 rounded-full font-medium text-sm tracking-[0.01em] bg-plum text-bg border-none cursor-pointer transition-[transform,background,opacity] duration-200 hover:enabled:-translate-y-px hover:enabled:bg-plum-2 disabled:opacity-60 disabled:cursor-not-allowed max-[640px]:justify-center"
        onClick={onPay}
        disabled={paying}
      >
